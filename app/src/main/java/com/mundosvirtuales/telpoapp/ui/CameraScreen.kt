...
/**
 * Composable principal que hostea toda la pantalla. Recibe el ViewModel que contiene toda la lógica
 * y el estado.
 */
@Composable
fun CameraScreen(viewModel: CameraViewModel) {
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        ...
        // --- 1.5. Overlay de Detección ---
        DetectionOverlay(detections = state.detections, previewWidth = 640, previewHeight = 480)
...
/**
 * Composable que envuelve la vista XML de la cámara (CameraViewInterface). Este es el "puente"
 * entre Compose y el sistema de Vistas clásico.
 */
@Composable
fun CameraPreview(viewModel: CameraViewModel, onLongPress: () -> Unit) {
    AndroidView(
            ...
            modifier =
                    Modifier.fillMaxSize().pointerInput(Unit) { // Añade el detector de gestos
...
/**
 * Dibuja todos los controles (botones, sliders) sobre la cámara. Es un Composable "tonto" que solo
 * reacciona al 'state'.
 */
@Composable
fun CameraOverlay(
...
fun TopControls(
    ...
    Row(
        ...
        Switch(checked = isCameraOn, onCheckedChange = onCameraToggle)
    ...
    Column(modifier = Modifier.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceEvenly) {
            if (state.supportsBrightness) {
                Button(onClick = onBrightnessClick) { Text("Brightness") }
            }

            if (state.supportsContrast) {
                Button(onClick = onContrastClick) { Text("Contrast") }
            }
        }
...
@Composable
fun SettingSlider(title: String, value: Int, onValueChange: (Int) -> Unit) {
    Card(
            modifier = Modifier.fillMaxWidth(),
            colors =
...
/**
 * Simula el layout de CameraScreen (un Box) con un fondo negro donde iría el AndroidView de la
 * cámara.
 */
@Composable
private fun PreviewScreenContainer(state: CameraState) {
...
/** Preview para el estado "Camera Off". Solo debería mostrar el Switch de encendido. */
@Preview(name = "Camera Off", showBackground = true)
@Composable
private fun CameraOffPreview() {
    PreviewScreenContainer(state = CameraState(isCameraOn = false))
}

/** Preview para el estado "Camera On". Muestra el botón de grabar y los botones de herramientas. */
...
/** Preview para el estado "Camera On" y "Recording". Muestra el botón de grabar en color rojo. */
...
/** Preview para el estado "Sliders Visible". Muestra el slider de Brillo. */
...
@Composable
fun DetectionOverlay(detections: List<DetectionResult>, previewWidth: Int, previewHeight: Int) {
